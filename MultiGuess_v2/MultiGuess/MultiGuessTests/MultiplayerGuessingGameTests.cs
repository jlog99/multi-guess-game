...
namespace MultiGuessTests
{
    ...
    {
        private Mock<IVocabularyChecker> mockVocabularyChecker;
        private MultiplayerGuessingGame game;

        [SetUp]
        public void Setup()
        {
            mockVocabularyChecker = new Mock<IVocabularyChecker>();
            mockVocabularyChecker.Setup(vc => vc.Exists(It.IsAny<string>())).Returns(true);

            var words = new List<string> { "poker", "cover", "pesto" };
            game = new MultiplayerGuessingGame(words);
        }
        ...
        {
            mockVocabularyChecker.Setup(vc => vc.Exists("xxxxx")).Returns(false);
